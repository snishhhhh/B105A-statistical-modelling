...
# Import dataset
# Reads the CSV from the same folder as this script — works for anyone
# who downloads this repo, no file.choose() dialog needed.
...
# Reduced Model Comparison (Section 6.4)
# A smaller model using only the strongest, stable predictors,
# to check whether the full model's complexity is actually necessary

reduced_model <- glm(
  Revenue ~ PageValues + ExitRates + ProductRelated_Duration + Month + VisitorType,
  family = binomial,
  data = model_data
)
...
# Likelihood ratio test: does the full model fit significantly
# better than the reduced model?
anova(reduced_model, logistic_model, test = "Chisq")

# Compare AIC directly: lower AIC = more efficient model
